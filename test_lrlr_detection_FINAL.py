...
rec_data_names = [
    # NOTE I included mix here even though it is eyes open because it is not trained on
    "v2_LRLR_once_3_mix",
    "v2_LRLR_once_4_mix",
    "v2_LRLR_once_5_mix",
...
def load_custom_data(session_folder_path):
    ...
    try:
        ...
        data_shape_on_save = session_info_loaded.get(
            'data_shape_on_save', 'samples_first')
        original_column_names = list(session_info_loaded.get(
            'column_names', [f'Ch{j+1}' for j in range(num_channels)]))
        ...
        if data_shape_on_save == 'channels_first':
            if loaded_flat_data.size % num_channels == 0:
                ...
                reshaped_data_channels_first = loaded_flat_data.reshape(
                    num_channels, num_samples_loaded_total)
                current_data_array = reshaped_data_channels_first.T
                # Start with original channel names
                ...
                block_counts = metadata_loaded.get(
                    'data_block_sample_counts', None)
                target_event_transitions = metadata_loaded.get(
                    'target_event_transitions', None)

                if block_ts is not None and block_counts is not None and len(block_ts) > 0:
                    # Validate counts match data length
                    if sum(block_counts) == current_data_array.shape[0]:
                        sample_timestamps = np.concatenate(
                            [np.full(int(cnt), float(ts)) for ts, cnt in zip(block_ts, block_counts)])
                        current_data_array = np.column_stack(
                            (sample_timestamps, current_data_array))
                        ...
                        if target_event_transitions is not None and len(target_event_transitions) > 0:
                            target_event_values = np.full(
                                len(sample_timestamps), False, dtype=bool)
                            ...
                            current_data_array = np.column_stack(
                                (current_data_array[:, 0], target_event_values, current_data_array[:, 1:]))
                            final_column_names.insert(1, "TargetEvent")
                        else:
                            print(
                                "Note: No target event transitions found in metadata or transitions array is empty.")
                    else:
                        print(
                            "Warning: Sum of block_counts does not match data length. Timestamps/Events not prepended.")
                else:
                    print(
                        "Note: data_block_timestamps or data_block_sample_counts not found or empty in metadata. Timestamps/Events not prepended.")

                processed_data = current_data_array
            else:
                print(
                    f"Error: Cannot reshape data saved as 'channels_first'. Total elements ({loaded_flat_data.size}) not divisible by num_channels ({num_channels}).")
                return None, session_info_loaded

        else:  # Assuming 'samples_first' or old format
            if loaded_flat_data.size % num_channels == 0:
                num_samples_loaded = loaded_flat_data.size // num_channels
                processed_data = loaded_flat_data.reshape(
                    num_samples_loaded, num_channels)
                final_column_names = list(original_column_names)
            else:
                print(
                    f"Error: Cannot reshape data saved as 'samples_first'. Total elements ({loaded_flat_data.size}) not divisible by num_columns ({num_channels}).")
                return None, session_info_loaded
        ...
        print(
            f"Error processing data from {data_filepath} or applying metadata: {e}")
        import traceback
        traceback.print_exc()
        return None, session_info_loaded
            ...
            if loaded_data.shape[0] > 0:
                ...
                for row in loaded_data[:5, :]:
                    ...
                    for i, item in enumerate(row):
                        col_name = display_column_names[i] if i < len(
                            display_column_names) else ""
                        if col_name == "Timestamp":
                            # Timestamp with 2 decimal places
                            ...
                            formatted_row.append(f"{item:.3f}" if not np.isnan(
                                item) else "NaN")  # Floats with 3 decimal places
                        else:
                            formatted_row.append(str(item))
...
def filter_signal_data(data, srate, mft=5, lowcut=0.5, highcut=15, artifact_threshold=125, apply_ica=True):
    ...
    for channel in range(data.shape[1]):
        ...
        if np.any(artifacts):
            ...
            segments = np.split(artifact_indices, np.where(
                np.diff(artifact_indices) > 1)[0] + 1)
            ...
            for segment in segments:
                if len(segment) > 0:
                    ...
                    for i, idx in enumerate(segment):
                        ...
                        data[idx, channel] = pre_value * \
                            (1 - weight) + post_value * weight
...
def _preprocess_input_for_model(data_segment_raw, model_srate_for_filter=118, mft=5, lowcut=0.5, highcut=15, artifact_threshold=125):
    ...
    try:
        data = filter_signal_data(data, srate=model_srate_for_filter, mft=mft,
                                  lowcut=lowcut, highcut=highcut, artifact_threshold=artifact_threshold)
    except ValueError as e:
        print(
            f"LSTM Preprocessing Error: Bandpass filtering failed: {e}. Check highcut vs model_srate_for_filter/2.")
...
def detect_lrlr_window_FINAL_MODEL(eog_data, srate, model_path=BEST_MODEL_PATH, detection_threshold=MODEL_THRESHOLD_BEST):
    ...
    if eog_data.shape[0] < MODEL_SAMPLE_LENGTH:
        print(
            f"Warning: EOG data too short for ({eog_data.shape[0]} samples, need {MODEL_SAMPLE_LENGTH}).")
        return False, 0.0

    if eog_data.shape[1] < 2:
        print(
            f"Warning: EOG data has fewer than 2 channels ({eog_data.shape[1]}). requires 2.")
        return False, 0.0
    ...
    preprocessed_segment = _preprocess_input_for_model(
        eog_segment_raw, model_srate_for_filter=118)
...
def test1():
    ...
    for filepath in rec_data_names:
        ...
        print(f"\nLoading data from: {SESSION_FOLDER_PATH}")
        ...
        (is_lrlr, score) = get_lrlr(eog_data)
        end_time = time.time()

        # Print results
        execution_time = end_time - start_time
        print(f"LRLR detected: {is_lrlr}, Score: {score:.4f}")
        ...
        print(
            f"Average value of last 750 target events: {np.mean(last_750):.4f}")
